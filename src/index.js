...
functions.cloudEvent('processVmEvent', async (cloudEvent) => {
  ...
  const instanceId = auditLog.resource?.labels?.instance_id || 'Unknown Instance';
  const zone = auditLog.resource?.labels?.zone || 'Unknown Zone';
  
  // Who performed the action
  const creator = auditLog.protoPayload?.authenticationInfo?.principalEmail || 'Unknown User';
  
  // Resource name (usually the actual VM name rather than numeric ID)
  const resourceName = auditLog.protoPayload?.resourceName || `instances/${instanceId}`;
  ...
  if (!webhookUrl) {
    console.warn(`No webhook configured for project ${projectId} and no default webhook found. Exiting.`);
    return;
  }
  ...
  try {
    ...
    if (!response.ok) {
      ...
      console.error(`Teams Webhook responded with status ${response.status}: ${errorText}`);
    } else {
